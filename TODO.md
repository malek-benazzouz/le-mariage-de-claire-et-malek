# TODO

- customize favicon and browser tab title

## Header

- make subheader "semi-sticky" (appear on scroll-up), or sticky otherwise
- handle responsiveness, alignment, positioning in a better way (a bit messy today + broken on small mobiles)
- add decorations (hearts?) between nav links
- in small view, since the nav is collapsed on click, it scrolls too far

## Carousel

## Countdown

## Intro

## Couple pres

- add background
- add animation (2 cards getting closer while scrolling)

## Program

- add a button that opens a popin for detailed content for each card
- design a vertical timeline (see http://onelove.catanisthemes.com/home-onepage-02/)
- add an icon on each card
- [bonus] each popin has a printable version
- content:
  - card 1: mairie + plan + où se garer
  - card 2: how to go to villa. fête + plan + detailed planning
  - card 3: brunch + plan + what to bring: maillots de bain / jeux de société / jeux de plein air (molkky, pétanque,
    badmington, ballon etc.)

## Accommodation

## RSVP

- content:
  - présent ?
  - +1 ?
  - photo étant petit

## Money pot

- create money pot and add link to it

## Bonus image

- add picture frame to image

## Footer

## Design / Style

- add animations + customize style. Ideas:
  - http://preview.themeforest.net/item/onelove-the-elegant-clean-multipurpose-wedding-wordpress-theme/full_screen_preview/20235980
  - https://www.julie-bast.fr/

- Adapt responsiveness when needed

------------------------------------------------------------------------------

# DONE

- force https redirection -> done via .htaccess file
- fallback to index.html in case of route not found (https://angular.io/guide/deployment#production-servers) -> done via
  .htaccess file
- set countdown to real date
